...
are expecting. It is expressed as two arrays of :code:`bedrock_dependency`
...
a :code:`io` entry and an :code:`sdskv_ph` entry. Our client has no dependency.
...
Finally the *flag* may be :code:`BEDROCK_OPTIONAL` for optional dependencies,
:code:`BEDROCK_REQUIRED` for a required dependencies,
:code:`BEDROCK_ARRAY` for an array of dependencies
...
a :code:`bedrock_args_t` handle from which we can extrat various configuration
parameters, including the Margo instance, the provider id (in provider registration),
the Argobots pool (same), the configuration (:code:`config` field
from the JSON configuration of the provider or the client), the name of the provider/client,
as well as its dependencies using :code:`bedrock_args_get_num_dependencies` and
...
.. important::
   At the moment, Bedrock initializes clients before providers, hence clients cannot
   depend on providers. Clients can only depend on provider handles to SSG groups, ABT-IO
   instances, other clients defined earlier, and provider handles.

Registering the module
----------------------

The last things to add to our library is to fill out a :code:`bedrock_module` structure
with our callbacks and dependency array, and to call :code:`BEDROCK_REGISTER_MODULE(module_a, ModuleA)`.
The first argument of this macro is the name under which we want our module to be known.
The second is the name of the :code:`bedrock_module` structure we have built.

.. note::
   All the above functions as well as the dependency array and the module structure instance
   can safely be declared :code:`static`. The only symbole required by the Bedrock to
   work with the library will be generated by the :code:`BEDROCK_REGISTER_MODULE` macro.
